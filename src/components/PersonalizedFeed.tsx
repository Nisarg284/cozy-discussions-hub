...
const PersonalizedFeed: React.FC<PersonalizedFeedProps> = ({ 
  ...
  const { getPersonalizedFeed, vote } = useRedditApi();
  ...
  const {
    ...
    queryKey: ["personalizedFeed", sort, isAuthenticated],
    queryFn: ({ pageParam }) => getPersonalizedFeed(sort, pageParam),
  ...
  if (isError) {
    return (
      <div className="text-center py-10">
        <p className="text-destructive mb-4">Failed to load personalized feed</p>
  ...
  if (posts.length === 0) {
    return (
      <div className="text-center py-10">
        <p className="text-muted-foreground mb-2">No posts found in your personalized feed</p>
        <p className="text-muted-foreground text-sm">Try subscribing to more subreddits</p>
      </div>
    );
  }
